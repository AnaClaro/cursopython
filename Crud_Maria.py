...
from tkinter import ttk, messagebox, filedialog #from, porque está importando duas ferramentas que estão dentro do tkinter.
import json
...
# Abre a janela para selecionar onde salvar o arquivo
    arquivo = filedialog.asksaveasfilename(defaultextension=".json", filetypes = [("Arquivos JSON", "*.json")], title="Salvar lista de pets como JSON")
...
btn_salvar_para_json = ttk.Button(frame_botoes, text="Salvar Json", command=salvar_para_json)
btn_salvar_para_json.grid(row=0,column=5,padx=5)
...
#Comando bind fica monitorando o que acontece e nesse comando vai selecionar o que aparecer
